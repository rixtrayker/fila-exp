...
class ClientManager
{
    ...
    private static function prepareData(): void
    {
        ...
        if (auth()->user()->hasRole('district-manager')) {
            ...
            foreach ($dates as $i => $date) {
                $day = PlanDataService::DAYS[$i];
                $dayClients = $plannedClients->filter(
                    fn ($visit) => $visit->visit_date->toDateString() === $date,
                );
                ...
                self::$clients[$day] = $dayClients
                    ...
                    ->toArray();
                self::$clients["{$day}-all"] = self::$clients[$day];
    ...
    public static function searchClients(string $search, string $day = null): array
    {
        $query = self::searchQuery($search);

        if (auth()->user()->hasRole('district-manager') && $day) {
            $query->whereKey(array_keys(self::getClients('all', $day)));
        }
