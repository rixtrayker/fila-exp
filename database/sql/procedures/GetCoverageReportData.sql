...
BEGIN
    ...
    LEFT JOIN (
        SELECT 
            ow.user_id,
            COUNT(DISTINCT DATE(ow.time_from)) as office_work_count
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            SELECT 
                ow.user_id, 
                DATE(ow.time_from) as busy_date
            FROM office_works ow
            WHERE DATE(ow.time_from) BETWEEN v_from_date AND v_to_date
        ...
        WHERE DAYOFWEEK(busy_date) NOT IN (1, 7) -- Only count weekdays
          AND busy_date NOT IN (
              SELECT DATE(oh.date) 
              FROM official_holidays oh 
              WHERE DATE(oh.date) = busy_date
          )
        GROUP BY user_id
